...
@st.cache_resource
def get_vector_store(folder_path: str = "./root_rules_data", db_dir: str = "./root_vector_db"):
    """
    ...
    print(f"🤖 Vector database not found. Scanning '{folder_path}' to build a new one...")
    ...
    if not documents:
        ...
        documents.append(Document(page_content="# Rule 1.1.2 Use of CANNOT\nThe term CANNOT is absolute.", metadata={"source": "core_rules.md"}))
...
def initialize_directory_rules_bot(folder_path: str = "./root_rules_data", db_dir: str = "./root_vector_db"):
    vector_store = get_vector_store(folder_path=folder_path, db_dir=db_dir)
    
    active_key = st.secrets.get("GEMINI_API_KEY") or os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY")
    
    if not active_key:
        raise ValueError("❌ GEMINI_API_KEY is not set in Streamlit Secrets, environment variables, or .env file!")
    ...
    def run_rag_pipeline(user_query: str):
        ...
        print(f"📖 [Debug] Reading context from: {set(doc.metadata.get('source') for doc in docs)}")
